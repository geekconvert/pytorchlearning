...
for epoch in range(10):
    model.train() # This also apply to the to the whole structure, meaning that it will say model.train.
    resnet50_model.eval()
    ...
    model.eval() # When we put the model into evaluation mode, this is applied recursively to the whole model structure.
    val_loss_sum = 0
    val_accurate = 0
    val_sum = 0
    with torch.no_grad():
        for X,y in val_dataloader:
            ...
            predictions = torch.sigmoid(outputs) > 0.5 # This is required because the outputs here are on a linear scale. And we want to turn them into probabilities between 0 and 1. So we need to manually apply the sigmoid function.
            accurate = (predictions == y).sum().item()
            val_accurate+=accurate
            val_sum+=y.size(0)
